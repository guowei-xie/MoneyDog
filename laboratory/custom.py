...
import pandas as pd
from laboratory.singleK import is_limit, is_one_board, is_t_board
...
def is_limit_board_after_volume_consolidation_v2(stock_code: str, daily_bars: pd.DataFrame, n: int = 5, m: int = 10, k: int = 2, r: float = 0.9, p: int = 10, l: float = -0.03, h: float = 0.1, w: float = 0.05) -> bool:
    f"""
    判断是否符合涨停后缩量盘整图形要求
    Args:
        stock_code: 股票代码
    ...
    if _is_exist_one_board(stock_code, daily_bars, m) or _is_exist_t_board(stock_code, daily_bars, m):
        return False
...
def _is_exist_t_board(stock_code: str, daily_bars: pd.DataFrame, m: int = 10) -> bool:
    """
