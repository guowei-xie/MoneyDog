...
- **Web 控制台**: FastAPI + Vue3 单页前端（`web/`），含总览仪表盘、回测配置/运行、交互式收益曲线与 K 线、交易/持仓钻取、策略对比、行情浏览
...
- **数据管理**: 使用 DuckDB 本地库存储日线、分钟线与指数（表：`daily_1day` / `daily_1min` / `index_daily` / `stock_list`，交易日历由 `daily_1day` 派生），路径由 `config.ini` 的 `data_path` 指定
...
├── (DuckDB 库由 config.ini [DATA] data_path 指定，可与项目分离，如 /path/to/stock.duckdb)
├── web/                  # Web 前端与 API 服务
│   ├── server.py         # FastAPI 应用入口（API + 托管前端构建产物）
│   ├── schemas.py        # API 数据模型（Pydantic）
│   ├── frontend/         # Vue3 + Vite 单页前端（构建产物 dist/ 由后端托管）
│   └── __init__.py
├── strategys/             # 策略模块
...
- 在 `config.ini` 的 `[DATA]` 段中设置 `data_path`，指向已准备好的 DuckDB 文件（本机绝对路径，可与项目分离，如 `/path/to/stock.duckdb`）
- 库内需包含以下表结构（`time` 列为**毫秒**级时间戳；读取时统一输出为秒级 `timestamp`）：

| 表名 | 说明 | 主要字段 |
|------|------|----------|
| `daily_1day` | 股票日 K | code, time, open, high, low, close, volume, amount |
| `daily_1min` | 股票 1 分钟 K | 同上 |
| `index_daily` | 指数日 K（如上证） | code 无后缀如 000001，其余同上 |
| `stock_list` | 股票池 | code（去重后作为主板股票池来源） |

- 无 `trade_calendar` 表：交易日历由 `daily_1day` 的交易日去重派生（避免依赖 akshare）
- 股票池由 `stock_list` 的 `code` 去重得到（仅保留 6 位数字+后缀的标准代码）；指数在策略中仍可用 `000001.SH`，代码内会映射为库中的 `000001`
...
[DATA]
# DuckDB 行情数据库路径（本机绝对路径；库内含 daily_1day / daily_1min / index_daily / stock_list，交易日历由 daily_1day 派生）
data_path = /path/to/stock.duckdb
```
...
# 评价指标：年化无风险利率（小数，如 0.02 表示 2%），用于夏普/索提诺
risk_free_rate = 0.0
# 评价指标：年化交易日数（夏普/年化收益/波动率的年化因子）
...
Web 控制台是一个 Vue3 + Vite 单页应用（`web/frontend/`），由 FastAPI 后端托管其构建产物。

**准备（首次或前端有改动时）**：需要 Node ≥ 18，在 `web/frontend/` 构建前端产物：

```bash
cd web/frontend
npm install
npm run build      # 生成 web/frontend/dist/，由后端托管
```
...
```bash
python -m web.server
# 或： uvicorn web.server:app --host 127.0.0.1 --port 8000
```

然后浏览器访问 `http://127.0.0.1:8000`。

> 注意：`utils.data` 在导入时即打开 DuckDB 连接，请勿加 `--reload`（监听子进程会二次导入并争抢数据库文件锁导致启动失败）。若 `dist/` 尚未构建，页面会提示先执行 `npm run build`。
...
# 终端 2：前端 Vite 开发服务器（已代理 /api 到 :8000）
cd web/frontend && npm run dev   # 访问 http://127.0.0.1:5173
```

Web 控制台功能：

- **总览仪表盘**：回测总数、运行状态、数据覆盖，最优回测（按夏普）与最近回测速览
- **回测**：策略/区间/资金费用/仓位/多线程配置，一键开始/中止；实时进度；刷新页面可恢复运行态；历史回测可排序表（查看/代码/记录/删除）。运行仅使用内存配置，不改写磁盘 `config.ini`
- **回测结果**：~13 项账户指标卡片；交互式收益/回撤/仓位曲线（ECharts，含上证基准）；交易明细与持仓时间线可排序筛选表；点击个股钻取带买卖点标记的 K 线；策略源码查看与记录 Excel 下载
- **策略对比**：多回测累计收益曲线叠加 + 关键指标对照
- **行情浏览**：浏览 DuckDB 中个股/指数的日线/分钟线 K 线与数据覆盖
...
- 回测结果页（`/runs/<id>`）展示：
  - ~13 项账户指标卡片（收益率、年化、最大回撤、夏普/索提诺/卡玛、Beta/Alpha、最大仓位等）
  - 交互式收益/回撤/仓位曲线（含上证基准），可缩放悬浮
  - 交易明细与持仓时间线（可排序/筛选），点击个股钻取带买卖点标记的 K 线
  - 账户/个股分析摘要、策略源码查看、记录 Excel 下载
- 历史回测（回测页）与总览仪表盘均可点击进入结果页；“删除”只移除记录索引，不删除结果文件
...
- **资金与收益**: 初始资金、期末总资产、总收益率、**年化收益率**（几何年化，按有效交易日样本）
- **交易统计（净收益口径）**: 交易次数、胜率、平均盈利率/亏损率、盈亏比、平均持仓天数
  - 单笔 `涨跌幅` 为**净收益**（已扣双向佣金 + 卖出印花税），胜率/盈亏比据此计算；另保留 `毛涨跌幅` 列供对照
  - 盈亏比对"无亏损交易"做除零保护，显示"无法计算"而非 NaN
  - 期末仍持仓的交易**不再静默丢弃**：按回测截止日收盘价做市值估算并以 `是否平仓` 列标记，同时输出
    「已平仓」与「含未平仓（期末市值）」两套汇总，避免评价被美化
- **风险与风险调整**: 最大回撤、年化波动率、**夏普比率**、**索提诺比率（Sortino）**、**卡玛比率（Calmar = 年化收益 / |最大回撤|）**
  - 夏普/索提诺使用可配置的无风险利率与年化交易日数（见下）；有效样本天数 < 60 时会提示"年化为外推结果，仅供参考"
- **相对基准（上证指数 000001.SH）**: 超额年化收益、Beta、Alpha（CAPM 年化口径）
- **仓位**: 每日持仓数量、最大持仓数、最大仓位资金占用率、空仓天数、账户/仓位曲线图

评价指标参数在 `config.ini [BACKTEST]` 配置：

- `risk_free_rate`：年化无风险利率（小数，如 `0.02` 表示 2%），默认 `0.0`
- `trading_days_per_year`：年化交易日数，默认 `252`

交易级与账户级指标的计算集中在 `laboratory/analyze.py`；其中交易级汇总由 `summarize_trades` /
`format_trade_summary` 统一实现，Web 前端（`web/server.py`）复用同一函数，保证前端与回测日志口径一致。
...
- `utils/data.py` 从 DuckDB 读取行情（表 `daily_1day` / `daily_1min` / `index_daily`，`time` 列为毫秒级；交易日历由 `daily_1day` 派生；分时时间戳已做北京时间 8 小时校正）
