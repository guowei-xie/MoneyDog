...
- **卖出阶段（分时）**
  - 组合化卖出：炸板清仓（优先级最高）、刚性止损（亏损达比例现价清仓）、分批止盈、分批止损。
...
- **条件1：近 N 日存在涨停，且最近一次涨停为首板/二板**
  - 在近 `limit_check_days=7` 个交易日内，必须能找到最近一次涨停日 `T`。
...
- **条件5：价格强于分时均价（成交额/成交量）**
  - `avg_price = sum(amount) / sum(volume)`（按数据单位换算为价格，代码中为 `total_amount / total_volume / 100`）。
...
> 说明：卖出只对持仓股生效，且必须有可用可卖数量。执行顺序：涨停保护 → 炸板清仓 → 刚性止损 → MACD 顶点过滤 → 组合A → 组合C。

- **卖出保护：当前涨停不卖出**
  - 若 `is_limit(stock_code, current_price, yesterday_close)` 为 True，直接不卖出。

#### 组合B：炸板清仓（优先级 1，对应 `_sell_combo_b_broken_limit`）
- **触发条件**
...
#### 刚性止损（优先级 2，对应 `_sell_rigid_stop_loss`）
- **触发条件**
  - 持仓亏损达到配置比例：`(cost_price - current_price) / cost_price >= rigid_stop_loss_rate(=0.07)`（即亏损 ≥7%）
- **动作**
  - 直接按 `available_volume` 现价清仓卖出（不依赖 MACD 顶点）。
...
- **触发条件**
  ...
  - 且通过“MACD 顶点过滤门槛”
  - （代码中不强制要求当前盈利，即无论盈亏只要满足上述条件即可触发分批止盈）
- **动作**
...
- **卖出信号**
  ...
  - `rigid_stop_loss_rate=0.07`：刚性止损阈值（亏损达到该比例时现价清仓，如 7%）
...
3. **刚性止损**
   - 当持仓亏损达到 `rigid_stop_loss_rate`（默认 7%）时立即现价清仓，不依赖 MACD 顶点，优先级在炸板清仓之后、组合A/C 之前。
4. **炸板清仓的偏保守**
   - 只要触及涨停后 3 分钟未回封就清仓，适合控制回撤，但可能会错过回封后的继续拉升。
5. **分批卖出机制**
   - 每天盘前会重置 `batch_sell_count`（默认 2），因此该分批是“日内分批”，不是跨日累计分批。
6. **组合A 不强制盈利**
   - 分批止盈（组合A）不要求当前价高于成本价，只要满足昨日涨幅/成交量/日内涨幅之一且通过 MACD 顶点过滤即可触发。
7. **日志与可观测性**
   - 代码中对关键触发点已记录日志（例如“炸板清仓/刚性止损/分批止盈/分批止损/选股命中”），回测/实盘建议配合日志回溯验证信号触发是否符合预期。
